...
| `get_uia_locators` | Export stable UIA locator recommendations plus explained bounded mapping between UIA and WPF identities | Reusable WPF/UIA IDs and identity on success; per-direction exact/heuristic/ambiguous/unmapped status, integer score, symbolic evidence, bounded candidates, ranked locators, and FlaUI snippets. A reverse WPF mapping failure is supplementary and does not discard valid UIA output. |
...
- Element handles: `resolve_element` returns an `elementId` handle for re-use across subsequent tool calls (and `find_elements` can include `elementId` values). Strict non-XPath ambiguity is a structured tool error whose bounded WPF/UIA candidates use the same deterministic ordering as locator `index`; ambiguous XPath segments retain their path-specific one-based indexing error. `uia_...` interactions validate XPath plus RuntimeId. `get_uia_locators` can map either direction using the shared scorer; UIA-origin results retain valid UIA output when reverse WPF mapping is unavailable, and known non-WPF windows complete as unmapped without injection. When it performs read-only current-occupant observation, it reports whether the source identity was verified, changed, or unverifiable and registers fresh IDs rather than projecting a stale ID onto a replacement. `wpf_...` handles are XPath-based and may go stale if the visual tree changes.
