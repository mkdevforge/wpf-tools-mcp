...
public sealed class ScreenshotCaptureDumps
{
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("WPF_TOOLS_MCP_DUMP_SCREENSHOTS"), "1", StringComparison.OrdinalIgnoreCase))
        {
            Assert.Pass("Set WPF_TOOLS_MCP_DUMP_SCREENSHOTS=1 to dump screenshots for manual comparison.");
        }
        ...
        try
        {
            var artifactsDir = Path.Combine(RepoRoot.Find(), "tests", "WpfToolsMcp.SnapshotTests", "_Artifacts");
