...
Usage:
  dotnet run --project tools/WpfToolsMcp.McpSmokeRunner -- [options]

Options:
  --server <path>         Path to WpfToolsMcp.McpServer.exe (auto-discovered if omitted)
...
""");
    ...
    private static string FindLatestServerExe()
    {
        ...
        var binRoot = Path.Combine(root, "src", "WpfToolsMcp.McpServer", "bin");
        if (!Directory.Exists(binRoot))
        {
            throw new DirectoryNotFoundException($"Could not find '{binRoot}'. Build WpfToolsMcp.McpServer first or specify --server.");
        }
    ...
    private static string FindRepoRootOrCwd()
    {
        ...
        {
            if (File.Exists(Path.Combine(dir.FullName, "WpfToolsMcp.slnx")))
            {
                return dir.FullName;
            }
