...
- 2026-05-06: Implemented in `21d82b3` and refined in `44cfc34` and `9d3b0b1`. Added internal `ElementTarget` as a locator-or-element-id value object, wired `click_element` MCP request construction through it, reused it in automation click resolution, and routed agent endpoint target validation through the same parser while preserving public DTOs.
- 2026-05-06: Final validation passed with 11 tests: `dotnet test tests\WpfToolsMcp.SnapshotTests\WpfToolsMcp.SnapshotTests.csproj -c Release --no-restore --filter "FullyQualifiedName~McpToolErrorsDesignTests|FullyQualifiedName~ElementTargetDesignTests|FullyQualifiedName~ClickElement_basic_button_updates_status_snapshot|FullyQualifiedName~ResolveElement_uia_then_click_by_elementId_snapshot"`. The command emits the known `System.Diagnostics.EventLog` version conflict warning.
...
- 2026-05-06: Self-review result: happy with the implementation. The parser is internal, keeps the wire DTOs unchanged, rejects missing/mixed targets before session lookup, requires a window handle for agent-side element-id targets, and does not inherit session window handles into element-id clicks.
